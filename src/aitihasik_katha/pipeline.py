...
def run_pipeline(topic: str | None = None) -> str | None:
    ...
    logger.info("Starting pipeline run_id=%s topic=%s", current_run_uuid, topic or "auto")
    ensure_directories(str(current_run_uuid))

    current_run_path = os.path.join(settings.RUNS_PATH, str(current_run_uuid))

    story = generate_story(topic=topic)
    logger.debug("Generated story:\n%s", story)

    audio_output_filepath = os.path.join(current_run_path, settings.AUDIO_PATH, "story.mp3")
    ...
    for idx, scene in enumerate(scenes):
        ...
        image_output_filepath = os.path.join(current_run_path, settings.IMAGE_PATH, f"image_{idx}.png")
        ...
        imageclip_output_filepath = os.path.join(current_run_path, settings.VIDEO_PATH, f"video_image_clip_{idx}.mp4")
    ...
    final_video_output_filepath = os.path.join(current_run_path, settings.OUTPUT_PATH, "final_video.mp4")
    ...
    if final_video_path:
        ...
        caption_path = Path(current_run_path) / settings.OUTPUT_PATH / "caption.txt"
        caption_path.write_text(caption, encoding="utf-8")

        folder_uri = upload_folder_to_gcs(settings.BUCKET, current_run_path, current_run_path)
    ...
    logger.warning("Pipeline completed without creating final video for run_id=%s", current_run_uuid)
    return None
